...
import { CreatePostGQL, GetPostsGQL, GetPostGQL, DeletePostGQL, UpdatePostGQL, CreatePostVars, UpdatePostVars } from '../post.gql';
...
export class PostGraphQLService {
  ...
  create(post: IPost): Observable<EntityResponseType> {
    return this.createPostGQL.mutate({ post } as CreatePostVars).pipe(map(result => this.graphQLUtils.toHttpResponse(result)));
  }
  ...
  update(post: IPost): Observable<EntityResponseType> {
    return this.updatePostGQL.mutate({ post } as UpdatePostVars).pipe(map(result => this.graphQLUtils.toHttpResponse(result)));
  }
